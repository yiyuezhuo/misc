...
\begin{tikzpicture}[node distance = 2cm, auto]
    ...
    \node [decision, left of=call_yolo, node distance=4cm] (decide_thres) {箱和滑块之一所有框最高信度小于阈值};
    \node [block, below of=decide_thres, node distance=4cm] (max_one_conf) {取箱和滑块最高信度的框};
    ...
    \node [block, below of=adjust, node distance=3cm](relative_loc) {根据框的中心估计滑块相对箱的位置};
    \node [block, right of=relative_loc, node distance=3cm](save) {保存结果};
    \node [startstop, below of=save, node distance=3cm](succ) {结束处理};
    \node [startstop, below of=max_one_conf, node distance=3cm](fail) {失败};
